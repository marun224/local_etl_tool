...
fn status(code: u16, body: &str) -> Answer {
    Answer {
        status: code,
        body: body.to_string(),
        headers: Vec::new(),
...
fn records(values: Vec<JsonValue>) -> Records<std::vec::IntoIter<Record>> {
    Records(
        values
            .into_iter()
